...
export const getWordOfDay = () => {
  ...
  const epochMs = new Date('January 1, 2022 01:00:00').valueOf()
  const now = Date.now()
  const msInDay = 86400000
  const index = Math.floor((now - epochMs) / msInDay)
  const nextday = (index + 1) * msInDay + epochMs
  const word_length = WORDS[index % WORDS.length].toUpperCase().length
...
export const { solution, solutionIndex, tomorrow, word_length} = getWordOfDay()
